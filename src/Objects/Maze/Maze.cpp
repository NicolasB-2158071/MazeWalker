...
m_wallsXZLocations{m_builder.getWallsXZLocations()}, m_wallSize{ m_builder.getWallSize()}
{
	...
	m_builder.readLocations("res/testMaze.txt");
...
bool Maze::isWallColision(glm::vec2 cameraPos) const
{
	...
	{
		// Between x and z intervals
		bool collisionX{pos.x < cameraPos.x && cameraPos.x < (pos.x + m_wallSize.x)};
		bool collisionZ{pos.y > cameraPos.y && (pos.y - m_wallSize.z) < cameraPos.y};
