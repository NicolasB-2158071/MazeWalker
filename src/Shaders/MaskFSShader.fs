...
vec3 calculatePointLight(int index, vec3 norm)
{
	...
	// Ambient
	vec3 ambient = pointLights[index].ambient * texture(material.texture_diffuse1, TexCoords).rgb;

	// Difuse
	vec3 lightDir = normalize(lightPosView - FragPos); // lightRayVector
	float diff = max(dot(norm, lightDir), 0.0); // Geeft hoek terug
	vec3 diffuse = pointLights[index].diffuse * diff * texture(material.texture_diffuse1, TexCoords).rgb; // Impact van licht
